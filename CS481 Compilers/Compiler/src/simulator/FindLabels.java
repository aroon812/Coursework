...
/** This class is passed an IRT program and makes a pass over the
 * entire tree, mapping labels to corresponding tree nodes, and
 * setting each Stm node's "next" link so the simulator has a 
 * sequence to follow.
 */
public class FindLabels {
	 ...
	 * subtree.  When visiting the right subtree, we use the next link passed
	 * to us (the SEQ's next node).  The only twist is that if the left subtree
	 * is the LABEL "main", we set next the null as we descend the right subtree.
	 * This ensures that after traversing main during a simulation we don't go 
	 * anywhere else once we're done.
	 * @param s  The SEQ node being visited
	 * @param next  Where to go after this SEQ node
	 */
	...
	public static void visitStm(Stm s, Stm next) {
		if (s instanceof SEQ) {
			...
			throw new Error("Unknown Stm node in visitStm: "+s.getClass().getName());
		}
	}
	...
	public static void visitExp(Exp e) {
		if (e instanceof BINOP) {
			...
			throw new Error("Unknown Exp node in visitExp: "+e.getClass().getName());
		}
	}
}
