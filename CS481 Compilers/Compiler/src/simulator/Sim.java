 ...
 * 
 * <li>The simulator does not support JUMPing into the STM portion of an ESEQ
 * node, as it's not clear where to go next after traversing the STM branch.
 * </ul>
 * 
...
public class Sim {
	...
	private static final int STACK_SIZE = 256; // 256 bytes (64 words) per stack, max
	...
	/**
	 * The constructor is passed the IRT tree to simulate. It makes a pass over the
	 * IRT tree to find all labels and stitch together "next" links for Stm nodes.
	 * Finally, it finds the first node to visit during a simulation, and leaves it
	 * in a field for later use
	 * 
	 * @param prog An IRT tree to "execute".
	 */
	public Sim(Stm prog) {
	...
	/**
	 * The run method begins the simulated execution of the IRT program passed to
	 * the constructor. The verbose flag controls whether or not it prints
	 * information about what it's doing as it runs the program. (If the flag is
	 * false, the output of print statements will still appear.) This method can be
	 * called repeatedly on the same tree.
	 * 
	 * @param verbose If true, simulator describes what it's doing as it executes
	 */
	...
	private void evalStm(LABEL s) {
		if (loud) {
			System.out.println("LABEL: " + s.label.toString());
		}
		evalStm(s.next());
	}

	/**
	 * JUMPs cause us to shift execution to the specified Label instead of following
	 * the "next" link.
	 */
	...
	/**
	 * Evaluate the Exps involved in the comparison, perform the specified test, and
	 * jump to the appropriate label instead of following "next". Note that this
	 * code doesn't handle the unsigned comparisons properly -- it performs signed
	 * comparisons instead since Java doesn't believe in unsigned values.
	 */
	private void evalStm(CJUMP s) {
		...
		case CJUMP.UGE:
			...
			break;
		default:
			throw new Error("Unsupported CJUMP encountered: " + s.relop);
		}
	...
	/**
	 * When simulating a MOVE, we only handle REG or MEM on the left-hand side. An
	 * IRT program shouldn't ever try to move a new value into $sp, so we watch for
	 * that case and warn the user if they attempt it.
	 */
	private void evalStm(MOVE s) {
		...
		if (s.dst instanceof REG) {
			...
			// System.out.println("" + registers.size() + " registers");
			if (loud) {
		...
		// reality, necessarily, but the IRT program won't notice.
			...
			if (addr > STACK_SIZE) {
				...
				if (loud) {
					System.out.println("stack(" + stacks.size() + ")[" + addr + "] <-- " + value);
				}
			}
		} else {
			throw new Error("Unknown node in MOVE: " + s.dst.getClass().getName());
		}
		evalStm(s.next());
	}

	/**
	 * Evaluate the expression contained within the EXPR, then move to the next IRT
	 * statement.
	 */
	 ...
	 * instance variable so the CALL mechanism can retrieve it. There's nowhere to
	 * go after hitting a RETURN, so there's no evalStm() call. Instead, we'll start
	 * unwinding Java's call stack until we get back to the CALL node that got us
	 * here.
	 */
	...
	/**
	 * Dispatch an evalStm to the appropriate method. It would be better OO design
	 * if each of the Tree nodes knew how to evaluate itself, but the folks writing
	 * that code didn't imagine the IRT would be executable so we're forced to do
	 * all of this outside of the Tree classes.
	 */
	private void evalStm(Stm s) {
		...
		if (s instanceof SEQ) {
			...
			throw new Error("Unknown node in evalStm: " + s.getClass().getName());
		}
	}
	 ...
	 * result. Note that expression nodes aren't "stitched together" like statements
	 * are -- there's no notion of where to go next after evaluating an expression.
	 * Instead, we just recursively evaluate the expression tree and return a value.
	 * 
	 * @param e An IRT expression tree.
	 */
	...
	/**
	 * Both MEM and REG represent storage locations. We'll catch MEM and REG nodes
	 * that appear on the LHS of a MOVE node and handle them separately, so it's
	 * safe to assume that these expressions imply reads instead of writes. As
	 * above, we can tell stack from heap addresses by their magnitude.
	 */
	private int evalExp(MEM e) {
		...
		if (idx > STACK_SIZE) {
			...
			if (loud) {
				System.out.println("stack(" + stacks.size() + ")[" + idx + "] --> " + stacks.peek().read(idx));
			}
	...
	private int evalExp(REG e) {
		if (loud) {
			System.out.println("Reg " + e.reg + " --> " + registers.get(e.reg.toString()));
		}
	...
	/**
	 * We intercept NAME references higher in the tree, so we should never get one
	 * here.
	 */
	 ...
	 * arguments to the proper offsets in the stack. (We assume the first of these
	 * arguments is the static link.) We then push our new IRStack onto the stack of
	 * IRStacks and jump to the new method. When control returns, we pop the stack,
	 * retrieve the return value from the returnVal instance variable, and return it
	 * as the value of the CALL node.
	 */
	private int evalExp(CALL e) {
		int result;
		int offset = SL_OFFSET; // Offset of first arg in new stack -- the static link
		...
		for (ExpList a = e.args; a != null; a = a.tail) {
			...
			if (loud) {
				System.out.println("stack(" + (stacks.size() + 1) + ")[" + (offset) + "] <-arg- " + result);
			}
		...
		/*
		 * The arguments to the method are now in a fresh IRStack that's been pushed on
		 * to our stack of stacks. Time to execute the method. Look to see if it's a
		 * built-in (malloc, print, or exit), otherwise jump to the appropriate label.
		 */
		if (loud) {
		...
		stacks.pop();
		if (loud) {
			System.out.println("CALL to " + methodName + " returned " + returnVal);
		}
		return returnVal;
	}
	...
	private int evalExp(Exp e) {
		if (e instanceof BINOP) {
			...
			throw new Error("Unknown node in evalExp: " + e.getClass().getName());
		}
	}
}
...
class Memory {
	 ...
	 * starting address at which malloc should start grabbing simulated memory. If
	 * start isn't zero, we end up with some unusable memory.
	 * 
	 * @param size  Total size of the memory in words.
	 * @param start Address at which malloc starts grabbing memory.
	 */
	...
	public int read(int idx) {
		if (idx % 4 != 0) {
			System.err.println("Non-aligned mem access (read): " + idx);
		}
		return mem[idx / 4];
	}
	...
	public void write(int idx, int val) {
		if (idx % 4 != 0) {
			System.err.println("Non-aligned mem access (write): " + idx);
		}
		mem[idx / 4] = val;
	}
